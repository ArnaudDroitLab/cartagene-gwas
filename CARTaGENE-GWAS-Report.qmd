...
# renv::install(packages = c("bioc::AnnotationDbi",
#                           "bioc::org.Hs.eg.db",
...
-   **Inputs**:
    -   Cleaned **VCF** file of the chromosome 7 (where `HDAC-9` is present): `chr7.merged.clean.noMono.vcf.gz`^[All VCF files per chromosome are stored in `/mnt/projects_tn01/Cartagene/genotypage/imputation/imputation_merged/` folder.]. 
...
-   **Objective**: Extract annotated SNPs within `HDAC9` boundaries, as defined by the `hg38` reference genome. **Details**:
    -   Filter variants based on **Minor Allele Frequency (MAF)** $> 0.01$ (in other words, the SNP must be present in at least $1\%$ of the samples).
    -   SNPs were extracted using `bcftools view`. [**Alternative**: `plink2 --bfile your_dataset --chr 7 --from-bp 18086825 --to-bp 19002416`, but requires BIM files as input.]{fg="red"}.


-   **Bash command**:

``` bash
bcftools filter --regions chr6:29942470-29945884 --include 'MAF[0] > 0.01' /mnt/projects_tn01/Cartagene/genotypage/imputation/imputation_merged/chr6.merged.clean.noMono.vcf.gz -o ./data/variants/chr6.HLA-A.vcf
```
...
<!-- ``` bash -->
<!-- bcftools filter --regions chr6:29942470-32666657 --include 'MAF[0] > 0.01' /mnt/projects_tn01/Cartagene/genotypage/imputation/imputation_merged/chr6.merged.clean.noMono.vcf.gz -o ./data/variants/chr6.HLA-complex.vcf -->
<!-- ``` -->
...
- **Remark 5**: we provide below an alternative and more compact syntax to call `plink2`, assuming binary and compressed files: 

``` bash
plink2 --bfile mydata \
  --pheno phenotype.txt \
  --covar covar.txt --covar-name age \
...
plink2 --double-id \
  --vcf ./data/variants/chr6.HLA-A.vcf dosage=HDS \
  --pheno ./data/phenotypes/merge_phenos_PCs.txt \
  --pheno-name DMOTSCORE_mod \
  --covar ./data/phenotypes/merge_phenos_PCs.txt \
  --covar-name PC1-PC10 \
  --glm hide-covar \
  --out tables/DMOTSCORE_mod

  
```

``` bash
plink2 --double-id --vcf ./data/variants/chr6.HLA-A.vcf dosage=HDS --pheno ./data/phenotypes/merge_phenos_PCs.txt --pheno-name DMOTSCORE --glm allow-no-covars --out tables/HLA-A 

```

#### ii) Visualisations: Manhattan plots

- More customisation available in [Manhattan plot in R: a review](https://r-graph-gallery.com/101_Manhattan_plot.html)

```{r}
gwas_hla <- readr::read_delim("./tables/HLA-A.DMOTSCORE.glm.linear",
                              delim = "\t",
                              show_col_types = FALSE) |> 
  dplyr::rename(CHR="#CHROM") |> 
  dplyr::mutate(SNP=paste0("rs", dplyr::row_number()))

qqman::manhattan(gwas_hla,
                 chr="CHR", bp="POS", p="P", snp = "SNP", 
                 annotatePval = 0.01)
