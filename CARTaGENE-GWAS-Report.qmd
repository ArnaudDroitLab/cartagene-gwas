...
bcftools filter --regions chr7:19002000-19002414 --include 'MAF[0] > 0.01' ./data/genotypages/chr7.merged.clean.noMono.vcf.gz -o ./data/variants/chr7.test.vcf
...
gwas_hla <- readr::read_delim("./tables/HDAC-9/HDAC-9_CVMACE_POST_2025-04-07.CVMACE_POST.glm.logistic.hybrid",
                              delim = "\t",
                              show_col_types = FALSE) |> 
...
# visualise qqplots
qqplot_pvalues <- ggplot(gwas_hla, mapping = aes(sample = P)) +
...
```{r}
#| label: gwas-histogram-visu

# Histogram of p-values distribution
histogram_pvalue <- ggplot(gwas_hla) +
...
title_plot <- cowplot::ggdraw() + 
  cowplot::draw_label("GWAS visualisations", 
                      fontface = "bold", x = 0.4, hjust = 0, size=20)

cowplot::plot_grid(title_plot, combined_plot,
