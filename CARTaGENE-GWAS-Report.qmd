...
- The response variables `osteopenia` and `osteoporosis` have been computed following these rules [@lst-osteopenie-computation], with the resulting contingency tables reported in @tbl-osteopenie-computation.
  ...
  1.  Exclude patients with `osteosecondaire==1`.^[Currently done by assigning all corresponding observations a missing value. Unfortunately, no option in `plink2` to subset the dataset, discarding irrelevant observations, however, would not recommend using hard-coded, fake `NA` values instead.]{fg="red"}
  2.  `osteopenia` is `case: 1` if `DMOTSCORE_mod <= -1.5` and `control` elsewhere^[`DMOTSCORE_mod` represents the measurement of bone mineral density at the calcaneus. While no universally accepted threshold exists for the diagnosis of osteopenia or osteoporosis, we adopt the conventional cut-off of `-1.5`.].
...
- **Fracture events**[Advise to code binary event variables as Boolean, rather than numeric.]{fg="red"}
...
cartagene_phenodata <- cartagene_phenodata |> 
  mutate(across(.cols = all_of(c("FXALL_POST", "FXALL_PRE1", "FXMOF_POST")),
                as.logical), 
         FXALL = FXALL_POST | FXALL_PRE1)
...
labels_OSTEO <- list(`post fracture` = as.character(cartagene_phenodata$IID[cartagene_phenodata$FXALL_POST==1]), 
                          `pre facture` = as.character(cartagene_phenodata$IID[cartagene_phenodata$FXALL_PRE1==1]), 
                          `osteoporotic fracture` = as.character(cartagene_phenodata$IID[cartagene_phenodata$FXMOF_POST==1]))
# Chart
venn.diagram(
        x = labels_OSTEO,
...
- The variables `CVALL_POST` and `CVMACE_POST` correspond to the occurrence of any cardiovascular event and **major adverse cardiovascular events** (MACE), including myocardial infarction, stroke, or cardiovascular death events, is the cognate subset of the most life-threating heart conditions. [An extended follow-up dataset is available for `CVALL_POST`; however, it is not currently accessible.]{fg="red"}

- The variable `MCASGLOBAL_PRE_M` captures ischaemic heart disease events, at baseline.
...
cartagene_phenodata <- cartagene_phenodata |> 
  mutate(across(.cols = all_of(c("CVALL_POST", "CVMACE_POST", "MCASGLOBAL_PRE_M", "AVCGLOBAL_PRE_M")),
                as.logical))
...
-   **Objective**: Extract annotated SNPs within `HDAC9` boundaries, as defined by the `hg38` reference genome. **Details**:
    ...
    -   SNPs were extracted using `bcftools filter`. [Recommended to use `bcftools view` when extracting per region domains or even `plink2 --bfile your_dataset --chr 7 --from-bp 18086825 --to-bp 19002416` to avoid relying on another tool.]{fg="red"}.
    -   No explicit mention of the reference genome used in the metadata lines of the VCF files.[It seems that among the 5 genotype arrays merged in @sec-white-selection, at least one genotype, namely `gsa.17k.final.hg19.bim,` has been mapped thanks to `GRCh37 (hg19)` reference instead of more recent `hg38 (GRCh38, 2013)` version.]
...
# old command
/home/galgen01/programs/bcftools/bin/bcftools filter --regions chr7:18086825-19002416 /mnt/projects_tn01/Cartagene/genotypage/imputation/imputation_merged/chr7.merged.clean.noMono.vcf.gz  -o /mnt/projects_tn01/Cartagene/analyses/variants_extraction/chr7.merged.clean.noMono.extracted_variants.HDAC9.vcf
```
...
AnnotationDbi::saveDb(txdb, "data/genome_builds/txdb_human_gencode_v42.sqlite")
```
...
``` bash
./bin/bcftools/bin/ ./bcftools filter \
  --regions chr6:29942470-29945884 \
  --include 'MAF[0] > 0.01'
  /mnt/projects_tn01/Cartagene/genotypage/imputation/imputation_merged/chr6.merged.clean.noMono.vcf.gz \
  -o /mnt/projects_tn01/Cartagene/analyses/variants_extraction/chr6.HLA-A.vcf
```
