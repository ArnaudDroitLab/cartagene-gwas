...
ln -s ./data/genotypages/ /mnt/projects_tn01/Cartagene/cartagene-gwas/data/genotypages
```
...
cartagene_phenodata <- cartagene_phenodata |> 
  dplyr::mutate(DMOTSCORE_mod = dplyr::if_else(OSTEOSECONDAIRE ==1, NA, DMOTSCORE), 
                ...
                OSTEOPOROSIS = dplyr::case_when(DMOTSCORE_mod < -2.5 ~ 1, 
                                              DMOTSCORE_mod >= -2.5 ~ 0, 
                                              .default = NA)) |> 
  dplyr::mutate(across(all_of(c("OSTEOPONIA", "OSTEOPOROSIS")), as.logical))
...
test_cartagene_phenodata <- cartagene_phenodata |> 
  dplyr::mutate(CVMACE_POST=dplyr::if_else(as.numeric(CVMACE_POST)==0, 1, 2))

readr::write_tsv(test_cartagene_phenodata, 
                 "./data/phenotypes/merge_phenos_PCs_test.txt")
```
...
``` bash
bcftools filter --regions chr7:18086825-19002414 --include 'MAF[0] > 0.01' ./data/genotypages/chr7.merged.clean.noMono.vcf.gz -o ./data/variants/chr7.HDAC9.vcf
```
...
```{r}
gwas_hla <- readr::read_delim("./tables/HDAC-9/HDAC-9_CVMACE_POST.CVMACE_POST.glm.logistic.hybrid",
                              delim = "\t",
                              show_col_types = FALSE) |> 
