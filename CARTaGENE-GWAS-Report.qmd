...
The code, main results (figures and tables), along with reports, are versionned in an unique place, on `GitHub`: 
...
-   [Check Human reference genome assembly, see @sec-HDAC9. Indeed, among the five genome arrays assembled in @sec-white-selection, most have been mapped against `GRCh37 (hg19)`, but at least one has been mapped against the recent `hg38 (GRCh38, 2013)` version, report to page 38 in @pelletier2022.]{fg="red"} 
...
- [Run sample size experiences prior to gene candidate studies, see @sec-sample-size.]{fg="red"}

## Introduction

### GWAS Litterature Review
...
- [Évaluation de l’imputation des données génétiques Canadiennes-Françaises](https://umontreal.scholaris.ca/bitstreams/83d64aa1-da20-4a6f-8ae0-9dcb0165417d/download), from @pelletier2022. **Key points**^[Comprehensive **Introduction section** reviewing the most critical challenges when coping with GWAS.]:
  
  - **Chapter 3**, named "Evaluation of genetic imputation in the French-Canadian founder population", pages 54-93. Details the underlying imputation strategy, derived from `TOPMed` (for Trans-Omics for Precision Medicine)^[[`PRED-LD`](https://github.com/pbagos/PRED-LD) (for prediction of linkage disequilibrium ), from @manios2025bb, is another CLI tool imputing and reporting automatically GWAS summary statistics, taking as inputs raw `VFC` files. The paper also reports state-of-the-art approaches for generating these quality control summary metrics of the imputation process]. 
  
  - **Page 45/61**: Describes imputation strategies, along with the preprocessing steps. Code for reproducing the analyses is [FC-imputation](https://github.com/JustinPelletier/FC-imputation). **Imputation performance** is much lower for SNPs exhibiting `MAF<0.01`, strongly promoting early removal of rare variants:
  
  - **Page 83/99**: Strong polymorphism in some regions, such as `HLA`.

  <!-- - Reach out [Julie Hussin](mailto:julie.hussin@umontreal.ca), or [LinkedIn profile](https://www.linkedin.com/in/hussinju/), who actually ran the PCA computations and global GWAS analyses. -->


![**GWAS of `CarTaGene` SNPs using TOPMed Merge-Impute strategy.** The [`TOPMed` Imputation Server](https://imputation.biodatacatalyst.nhlbi.nih.gov/) has been used. Manhattan plots depict the genome-wide association study (GWAS) results for variants, before and after imputation The figure is reproduced from [@pelletier2022, pp. 97-98]. The $y$-axis represents the $-\log_{10}(p)$
$p$-value association of each variant. [Variants with 
-values equal to zero are highlighted in green]{fg="green"}, while the [horizontal red line indicates the genome-wide *significance threshold*.]{fg="red"}. The total sample consisted of $27,422$ individuals, with the **GWAS focus being comparing ground-truth SNPs versus imputed SNPs.**](./figures/global-GWAS.png){#fig-gwas-global}
...
[^2]: Notably includes MHC, lactase region, and known inversions `8p23` and `17q21.31`.
...
#### Step 3: PCA Computation and Population Genetic Evaluation {#sec-PCA}
...
- **Remarks**:
  ...
  -   [**Remark 1**: use visualizations, such as *scree plots*, elbow point or/and *Tracy-Widom Test* to select the final number of PCs, instead of hard threshold, and on the other hand, scatter plots to identify latent structures. See details [here](https://www.linkedin.com/posts/joachim-schork_statistics-research-visualanalytics-activity-7251577056570785795-b6Rk) and [there](https://www.linkedin.com/feed/update/urn:li:activity:730919293617122508)]{fg="red"}.

  - **Remark 2**: The [`popgen` *'Calculating Basic Population Genetic Statistics from SNP Data'*](https://popgen.nescent.org/StartSNP.html#genetic-diversity-observed-and-expected-heterozygosity) vignette details how to generate statistics quantifying patterns of genetic diversity, such as LD and Hardy-Weinberg, in a given population. 
  The [`popgen` *'Calculating genetic differentiation and clustering methods from SNP data'* vignette](https://popgen.nescent.org/DifferentiationSNP.html) vignette describes both unsupervised, PCA-like and discriminatory approaches to identify latent genetic sub-populations.  
  Finally, the [`popgen` *'Individual Based Genetic Distance for SNP Data'*](https://popgen.nescent.org/2015-05-18-Dist-SNP.html) vignette reports different individual genetic distances, enabling to spot and trim individual outliers with a significant distinct genetic distribution.
  See also paper ['A benchmark study on current GWAS models in admixed populations'](https://doi.org/10.1093/bib/bbad437), from @yang2024bib for an unbiased and up-to-date comparison of statistical approaches to address within-patient heterogeneity.
  
  - **Remark 3**: To evaluate within-population diversity, geneticists typically report the level of *heterozygosity* as a general metric, see [`vcfR`'Genetic differentiation' Vignette](https://knausb.github.io/vcfR_documentation/genetic_differentiation.html). 

### Run GWAS analyses

#### Step 1: Phenotype feature extraction {#sec-phenotype-extraction}

Original phenotype annotations are available in folder `/mnt/projects_tn01/Cartagene/analyses/phenotypes`. Variables of interest are reported [here](./data/phenotypes/cartagene_response_variables.xlsx):

```{r}
#| label: tab-cartage-pheno-meta
...
knitr::include_graphics("figures/venn_diagram_fractures.png")
...
Finally, we save the updated phenotype dataset as a Tab-separated file, using `readr::write_tsv`, see @tbl-cartage-pheno-features-and-save for details:

```{r}
#| label: tbl-cartage-pheno-features-and-save
#| tbl-cap: Features of interest to regress on SNPs in `CarTaGene`. Besides, recent versions of `plink2` impose explicit encoding of categorical variables, encoding 1 as 'controls', and 2 as 'cases'.
#| tbl-subcap:
#|   - "Bone Damages"
...
[^8]: All curated VCF files per chromosome are stored in `./data/genotypages/` (linked symbolically with `/mnt/projects_tn01/Cartagene/genotypage/imputation/imputation_merged`).
...
<!-- ### Step 3: Quality Controls of VCF files -->

<!-- Package `vcfR` streamlines the automated generation of QC plots per chromosome, as reported in @fig-plot-qc-per-chrom: -->

<!-- ```{r} -->
<!-- #| label: fig-plot-qc-per-chrom -->
<!-- #| fig-cap: Quality control per chrosomsome, for a given variant. Specifically, **annotation data** is represented on the lowest panel, with features represented as dark red rectangles. Above is the **sequence track**, with [nucleotides in green]{fg="green"} while [uncalled nucleotides are in red]{fg="red"}. Last track summarizes the number of variants per window. See details [here](https://knausb.github.io/vcfR_documentation/visualization_1.html). -->
...
- See also [QC of Base Data: A Tutorial](https://choishingwan.github.io/PRS-Tutorial/base/).

## Results: GWAS Analyses

#### i) GLM and GWAS {#sec-GWAS}

-   **Objective**: Use of `PLINK2 + glm` with the first 10 principal components as covariates, see @lst-glm-bash, using logistic regression for categorical variables, and `lm` for continuous outcomes. [Note that in contrast with Geneviève's implementation, recent `plink2` versions automatically switch from **Standard Logistic Regression**, with potentially non-finite estimates, to logistic **Firth Hybrid Regression**, which guarantees finite and bias-reduced estimates. This was activated for instance. Automatically activated when the Case ratio over Control is below 0.1. With this new option, remark 1 is not anymore relevant (VIF inflation factor).]{fg="red"}
...
```{r}
#| label: fig-gwas-OR-visu
#| fig-cap: "Density plot of the OR (Odds Ratio) for variable `OSTEOPONIA`, and gene `HDAC-9`."
gwas_results <- readr::read_tsv("./tables/HDAC-9/HDAC-9_OSTEOPONIA_2025-04-07.OSTEOPONIA.glm.logistic.hybrid",
                                show_col_types = FALSE) 
...
### Retrieve automatically variants of interest, aka gene-candidate approach {#sec-post-GWAS}

- [A current guide to **candidate gene association** studies](https://www.cell.com/trends/genetics/abstract/S0168-9525), from @david2021tig. 

#### Review of data-driven approaches for automated selection of variants

This analysis is not formally a GWAS study, but rather a *gene-candidate* study, pre-selecting genes based on prior knowledge and/or intuition. Alternatively, we could consider a data-driven, and more agnostic approach, to refine the selection of gene candidates. Several approaches to that end have been implemented:
...
<!-- #### Automated annotation and mapping of variants with `VariantAnnotation` -->
...
### Include additional cohorts

- [Rare-variant aggregate association analysis using imputed data is a powerful approach](https://www.biorxiv.org/content/10.1101/2025.04.17.649394v1), from @liu2025, reports guidelines for including rare variants in GWAS analyses, starting from genotyping arrays derived from the **UK Biobank**

- Other public GWAS cohorts include 1000 Genomes2, African Genome Resource3, Asthma among African-ancestry Populations in the Americas (CAAPA)4, Genome Asia Pilot (GAsP)5, HAPMAP26, Haplotype Reference Consortium (HRC)7, Multi-ethnic HLA8, Southeast Asian Reference Database (SEAD)9, Trans-Omics for Precision Medicine (TOPMed)10, UK10K11, and the Westlake Biobank for Chinese (WBBC).

### Post-hoc GWAS

- Collect information on variants with LLM-like [`VarChat`](https://varchat.engenome.com/) application.

- Infer the most influential **effector genes** that drive loci changes and altered diseased phenotypes in GWAS studies using the `FLAMES` (Fine-mapped Locus Assessment Model of Effector geneS) data-driven framework, combining SNP-to-gene data (chromatin interaction mapping or eQTL mapping) and prior biological pathway information. Available as an open-source and popular [GitHub Python repo](https://github.com/Marijn-Schipper/FLAMES), from @gnanaolivu2025bb.

- To further extend `FLAMES`, you may harness [`TraitGym`](https://huggingface.co/datasets/songlab/TraitGym), a curated dataset of regulatory genetic variants causally related to 113 Mendelian and 83 complex traits, from @benegas2025.

### Explore non-coding and regulatory regions (from WES to WGS)
...
- Automatically extract and annotate variants, based on their locations, using [`VariantAnnotation::locateVariants()`](https://www.bioconductor.org/packages/release/bioc/vignettes/VariantAnnotation/inst/doc/VariantAnnotation.html#locating-variants-in-and-around-genes) function. Variants annotations notably encompass coding, UTRs, introns and promoters.
...
- The [VariantAnnotation::predictCoding](https://www.bioconductor.org/packages/release/bioc/vignettes/VariantAnnotation/inst/doc/VariantAnnotation.html#amino-acid-coding-changes) computes amino acid coding changes for non-synonymous variants, while the [`SIFT` and `PolyPhen`](https://www.bioconductor.org/packages/release/bioc/vignettes/VariantAnnotation/inst/doc/VariantAnnotation.html#sift-and-polyphen-databases)  web databases evaluate the impact of SNP substitution, based on the predicted level of changes of the 3D-protein reconfiguration.
...
- [Calculating and Analysing PRS](https://choishingwan.github.io/PRS-Tutorial/cal_prs/) offers a comprehensive tutorial on deriving polygenic scores using R and `plink2`, for the most popular approaches: `PLINK` (requires post-aggregation of ORs), `PRSice-2`, `LDPred-2` and `lassosum`. [Plotting the Results](https://choishingwan.github.io/PRS-Tutorial/plink_visual/) is the follow-up tutorial for representing PRS scores.
...
- [`FIVEx`](https://github.com/statgen/fivex) is a web-based interface to explore `CarTaGene` to visualize and query eQTL (quantitative trait loci) data in various ways. **Would need a stringent maintenance!!**
...
-   **NextFlow**: [`nf-GWAS` 'Nextflow pipeline'](https://genepi.github.io/nf-gwas/), from @schonherr2024, is actively maintained by Curie Bioinformatics Team, and includes the latest `plink2` facilities. Complementary tools include:

  - The `Nextflow` Seqera [`multiqc`](https://seqera.io/multiqc/) initiative collects within the same repository a variety of command-line tools, such as `bcftools` and `plink2`, with notably the [`vcftools`](https://github.com/MultiQC/MultiQC/tree/main/multiqc/modules/vcftools) and [`multivcfanalyzer`](https://github.com/MultiQC/MultiQC/tree/main/multiqc/modules/multivcfanalyzer) modules. Note however that these tools are optimised in Python.
  
  - [`PGSXplorer`](https://github.com/tutkuyaras/PGSXplorer) is a Nextflow pipeline automatically retrieving and carrying out quality controls and polygenic scores computation. The workflow, starting from a `VCF` file, includes checks and automated reporting for missing SNPs, individual genotype quality, MAF, HWE, relatedness, heterozygosity, and duplicate SNPs. Additionally, it outlines the GWAS QC and **imputation** components of the pipeline. It both returns single scores (`plink`, `SBayes`) and polygenic scores (`MUSSEL`).

-   **SnakeMake**: [`PopGLen` 'Snakemake' pipeline](https://academic.oup.com/bioinformatics/advance-article/doi/10.1093/bioinformatics/btaf105/8069456), from @nolen2025b. [Not for running GWAS analyses, but rather for evaluating the quality and impact of preprocessing and quality mapping at the population-level genome starting from `FASTQ` files.]{fg="red"}

### FASTQ and FASTA Quality Controls

- The 7th chapter entitled ["Quality Check, Processing and Alignment of High-throughput Sequencing Reads"](https://compgenomr.github.io/book/processingReads.html) introduces the fundamentals, using R packages, to process FASTA reads, and evaluate their quality with a variety of metrics ($k$-mer over-representation, sequence quality, percentage of duplicated reads, ..).
