...
- `GitHub` to version the code
- R project + activate `renv()` to create a reproducible and standalone computational environment.
...
- Bash configuration^[We use *symbolic links* to encapsulate the whole project, see [`ln -s` Post](https://www.linkedin.com/posts/%F0%9F%8E%AF-ming-tommy-tang-40650014_the-most-underrated-unix-command-for-bioinformatics-activity-7287842704753848321-qhe) for details.]
...
-   **Objective**: Remove SNPs variants associated with strong **linkage disequilibrium** using prior expert knowledge, see [here](http://dougspeed.com/wp-content/uploads/highld.txt) for details.[^1]
...
All the curated `VCF` files have been downloaded and processed by [Email Cartagene](mailto:access@cartagene.qc.ca). Preprocessing details are reported [here](https://cartagene.qc.ca/files/documents/other/Info_GeneticData3juillet2023.pdf)[^3]
...
-   **Objective**: Extract annotated SNPs within `HDAC9` boundaries, as defined by the `hg38` reference genome. **Details**:
    ...
    -   SNPs were extracted using `bcftools view`, with shell script [extract_variants.sh](./shell/extract_variants.sh)^[`nohup` option combined with `&` character enables running variants extraction in the background, even with terminal disrupting and closing.]: 
...
-   **Objective**: Use of `PLINK2 + glm` with the first 10 principal components as covariates, see @lst-glm-bash.
    -   **Fractures analysis**: `OSTEOPOROSIS`, `OSTEOPONIA` and `fractures` using logistic regression + `DMOTSCORE_mod` was analysed using `lm`.
...
For each gene-phenotype pair, we generate cognate **Manhattan plots**^[More customisation available in [Manhattan plot in R: a review](https://r-graph-gallery.com/101_Manhattan_plot.html)], and  $p$-values distributions, using *histograms*^[[How to interpret a $p$-value histogram](https://varianceexplained.org/statistics/interpreting-pvalue-histogram/)], and **QQplots**^[[How I Make QQ Plots Using `ggplot2`?](https://danielroelfs.com/blog/how-i-make-qq-plots-using-ggplot/)].
...
In total, **7 phenotypes, 2613 variants tested, and $18291$ pairwise tests** were performed, imposing the need for multiple test correction. A significance threshold of $2.7 \times 10^{-6}$ is suggested (for the raw $p-$ value), relying on standard FWER correction.
