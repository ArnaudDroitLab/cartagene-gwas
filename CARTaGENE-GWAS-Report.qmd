...
- Username: `bastienchassagnol`
- Password:`TyBkK2irX9Srh1XZdPdC`
...
## Step 1: Keep Eurasian genotypes 

Genotypes were split into five different genotyping arrays. Arrays were merged back together using Bash script `/mnt/projects_tn01/Cartagene/analyses/QC/merge_datasets.sh`, and command `--bmerge` of tool `plink2`. This command only keeps **SNPs** and matching **alleles** present in all datasets.

The corresponding `BAM` files (sequence alignments), `BED` files (genomic regions of interest), `BIM` (SNP information) and `FAM` (phenotype data, such as individuals and pedigree), are listed in folder `/mnt/projects_tn01/Cartagene/analyses/QC`, with prefix `merge_5_*` (for 5 genotypes concatenated). 
...
- [**Remark**: Also advised to exclude samples with low Genotyping Rate, below 0.98. Not done in this study.]{fg="red"}
...
- [**Remark**: If a SNP is under strong **natural selection**, such as SNPs involved in the HLA genes, they are likely to deviate from HWE due to balancing selection. If the SNP is biologically important, don't exclude it blindly!!]{fg="red"}
...
- [**Remark**: use visualizations, such as *scree plots*, elbow point or/and *Tracy-Widom Test* to select the final number of PCs, instead of hard thresholding]{fg="red"}.
...
See details [here](/mnt/projects_tn01/Cartagene/analyses/phenotypes/creation_pheno_input/) and ...
...
- *Objective*: Use of `PLINK2 + glm` with the first 10 principal components as covariates:
